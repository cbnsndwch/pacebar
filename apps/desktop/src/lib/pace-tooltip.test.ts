...
describe("formatDeficitText", () => {
  ...
  it("formats count deficit", () => {
    expect(formatDeficitText(15, { kind: "count", suffix: "requests" }, "used")).toBe(
      "15 requests in deficit",
    );
  });

  it("formats decimal count deficit without forced trailing zeros", () => {
    expect(formatDeficitText(4.5, { kind: "count", suffix: "requests" }, "used")).toBe(
      "4.5 requests in deficit",
    );
  });
  ...
  it("shows the first displayable count deficit", () => {
    expect(formatDeficitText(0.005, { kind: "count", suffix: "requests" }, "used")).toBe(
      "0.01 requests in deficit",
    );
  });
});
