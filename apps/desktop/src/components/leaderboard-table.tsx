...
function formatValue(entry: LeaderboardEntry, metric: LeaderboardMetric): string {
  switch (metric) {
    case "tokens":
      return `${formatTokens(entry.tokens_used)} tokens`;
    case "dollars":
      return `$${entry.dollars_spent.toFixed(2)}`;
    case "providers":
      return `${entry.providers_active} provider${entry.providers_active !== 1 ? "s" : ""}`;
    case "score":
      return `${entry.score.toFixed(0)} pts`;
  }
}
        ...
        return (
          <div
            ...
            className={cn(
              ...
              isMe ? "bg-primary/10 ring-1 ring-primary font-medium" : "hover:bg-muted/50",
            )}
          >
            ...
            <span className="tabular-nums text-muted-foreground">{formatValue(entry, metric)}</span>
          </div>
        );
      })}
    </div>
