(function () {
  ...
  function writeStateValue(ctx, key, value) {
    try {
      ...
      const sql =
        "INSERT OR REPLACE INTO ItemTable (key, value) VALUES ('" + key + "', '" + escaped + "');";
  ...
  function loadAuthState(ctx) {
    ...
    const sqliteMembershipType =
      typeof sqliteMembershipTypeRaw === "string"
    ...
    const hasDifferentSubjects =
      !!sqliteSubject && !!keychainSubject && sqliteSubject !== keychainSubject;
    ...
    if (sqliteAccessToken || sqliteRefreshToken) {
      if (
        ...
        sqliteLooksFree &&
        hasDifferentSubjects
      ) {
        ctx.host.log.info(
          "sqlite auth looks free and differs from keychain account; preferring keychain token",
        );
        return {
  ...
  function refreshToken(ctx, refreshTokenValue, source) {
    ...
    try {
      ...
      if (resp.status === 400 || resp.status === 401) {
        ...
        ctx.host.log.error(
          "refresh failed: status=" + resp.status + " shouldLogout=" + shouldLogout,
        );
        if (shouldLogout) {
  ...
  function probe(ctx) {
    ...
    const normalizedPlanName = typeof planName === "string" ? planName.toLowerCase() : "";

    const hasPlanUsage = !!usage.planUsage;
    const hasPlanUsageLimit =
      hasPlanUsage &&
    ...
    const needsRequestBasedFallback =
      usage.enabled !== false &&
      (!hasPlanUsage || planUsageLimitMissing) &&
      (normalizedPlanName === "enterprise" || normalizedPlanName === "team");
    ...
    const needsFallbackWithoutPlanInfo =
      usage.enabled !== false &&
      (!hasPlanUsage || planUsageLimitMissing) &&
    ...
    if (needsFallbackWithoutPlanInfo) {
      ctx.host.log.info(
        "plan info unavailable with missing planUsage, attempting REST usage API fallback",
      );
    ...
    const hasValidGrantData =
      hasCreditGrants && grantTotalCents > 0 && !isNaN(grantTotalCents) && !isNaN(grantUsedCents);
    ...
    const planUsed = hasPlanUsageLimit
      ...
      : 0;
    const computedPercentUsed = hasPlanUsageLimit && pu.limit > 0 ? (planUsed / pu.limit) * 100 : 0;
    const totalUsagePercent = hasTotalUsagePercent ? pu.totalPercentUsed : computedPercentUsed;
    ...
    if (isTeamAccount) {
      if (!hasPlanUsageLimit) {
        ctx.host.log.warn(
          "team-inferred account missing planUsage.limit, attempting REST usage API fallback",
        );
        return buildUnknownRequestBasedResult(ctx, accessToken, planName);
      }
      lines.push(
  ...
  globalThis.__pacebar_plugin = { id: "cursor", probe };
})();
