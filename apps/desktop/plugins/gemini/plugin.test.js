    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL) {
        return {
          status: 200,
          bodyText: JSON.stringify({
            projects: [{ projectId: "other-project" }, { projectId: "gen-lang-client-456" }],
          }),
        };
      }
      if (url === QUOTA_URL) {
  ...
  it("continues with existing token when refresh token is missing", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-888" }] }),
        };
      if (url === QUOTA_URL) {
        return {
          status: 200,
  ...
  it("continues with existing access token when oauth client creds are missing", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL) {
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-123" }] }),
        };
      }
      if (url === QUOTA_URL) {
  ...
  it("continues when refresh request throws and an access token already exists", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL) {
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-456" }] }),
        };
      }
      if (url === QUOTA_URL) {
        expect(opts.headers.Authorization).toBe("Bearer existing-token");
  ...
  it("continues with existing token when refresh returns non-2xx", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-777" }] }),
        };
      if (url === QUOTA_URL) {
        return {
          status: 200,
  ...
  it("ignores non-string oauth2.js payload and continues", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-666" }] }),
        };
      if (url === QUOTA_URL) {
        return {
          status: 200,
  ...
  it("skips proactive refresh when expiry_date is non-numeric", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-555" }] }),
        };
      if (url === QUOTA_URL) {
        return {
          status: 200,
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === QUOTA_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({
            buckets: [{ modelId: "other-model", remainingFraction: 0.2 }],
          }),
        };
      throw new Error("unexpected url: " + url);
    });
  ...
  it("throws when quota request is unauthorized and refresh cannot recover", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-789" }] }),
        };
  ...
  it("throws when quota request returns non-2xx", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-321" }] }),
        };
  ...
  it("throws when quota response JSON is invalid", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-111" }] }),
        };
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL) {
        return {
          status: 200,
          bodyText: JSON.stringify({
            projects: [{ projectId: "labeled-project", labels: { "generative-language": "1" } }],
          }),
        };
      }
      if (url === QUOTA_URL) {
  ...
  it("discovers oauth2.js via dynamic NVM version scanning", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === TOKEN_URL) {
        return {
          status: 200,
          bodyText: JSON.stringify({ access_token: "refreshed-token", expires_in: 3600 }),
        };
      }
      if (url === LOAD_CODE_ASSIST_URL) {
        return { status: 200, bodyText: JSON.stringify({ tier: "standard-tier" }) };
      ...
      if (url === PROJECTS_URL) {
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-nvm" }] }),
        };
      }
      if (url === QUOTA_URL) {
  ...
  it("discovers oauth2.js via dynamic fnm version scanning", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === TOKEN_URL) {
        return {
          status: 200,
          bodyText: JSON.stringify({ access_token: "fnm-token", expires_in: 3600 }),
        };
      }
      if (url === LOAD_CODE_ASSIST_URL) {
        return { status: 200, bodyText: JSON.stringify({ tier: "standard-tier" }) };
      ...
      if (url === PROJECTS_URL) {
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-fnm" }] }),
        };
      }
      if (url === QUOTA_URL) {
  ...
  it("discovers oauth2.js via nested package structure", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === TOKEN_URL) {
        return {
          status: 200,
          bodyText: JSON.stringify({ access_token: "nested-token", expires_in: 3600 }),
        };
      }
      if (url === LOAD_CODE_ASSIST_URL)
        return { status: 200, bodyText: JSON.stringify({ tier: "standard-tier" }) };
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-nested" }] }),
        };
      if (url === QUOTA_URL) {
  ...
  it("discovers oauth creds via Homebrew bundle chunks", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === TOKEN_URL) {
        ...
        return {
          status: 200,
          bodyText: JSON.stringify({ access_token: "brewed-token", expires_in: 3600 }),
        };
      }
      if (url === LOAD_CODE_ASSIST_URL)
        return { status: 200, bodyText: JSON.stringify({ tier: "standard-tier" }) };
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-brew" }] }),
        };
      if (url === QUOTA_URL) {
  ...
  it("logs warning when no oauth2.js is found at any path", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-warn" }] }),
        };
      if (url === QUOTA_URL) {
        return {
          status: 200,
  ...
  it("uses snake_case quota fields and still renders lines", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url === PROJECTS_URL)
        return {
          status: 200,
          bodyText: JSON.stringify({ projects: [{ projectId: "gen-lang-client-909" }] }),
        };
      if (url === QUOTA_URL) {
        return {
          status: 200,
