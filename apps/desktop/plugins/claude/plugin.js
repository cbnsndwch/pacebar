(function () {
  ...
  function probe(ctx) {
        ...
        if (!wasRateLimited && nowMs - lastUsageFetchMs < MIN_USAGE_FETCH_INTERVAL_MS) {
          ...
          ctx.host.log.info(
            "usage fetch skipped: last fetch was " +
              Math.round((nowMs - lastUsageFetchMs) / 1000) +
              "s ago (min interval " +
              MIN_USAGE_FETCH_INTERVAL_MS / 1000 +
              "s)",
          );
        } else {
          ...
          if (resp.status === 429) {
            ...
            const backoffMs =
              retryAfterSeconds !== null ? retryAfterSeconds * 1000 : DEFAULT_RATE_LIMIT_BACKOFF_MS;
            rateLimitedUntilMs = nowMs + backoffMs;
            data = cachedUsageData;
            ctx.host.log.warn(
              "usage rate limited (429), backing off for " + Math.round(backoffMs / 1000) + "s",
            );
      ...
      if (data.extra_usage && data.extra_usage.is_enabled) {
        ...
        if (typeof used === "number" && typeof limit === "number" && limit > 0) {
          ...
          lines.push(
            ctx.line.text({
              label: "Extra usage spent",
              value: "$" + String(ctx.fmt.dollars(used)),
            }),
          );
        }
      }
    ...
    if (rateLimited) {
      const retryText = retryAfterSeconds !== null ? fmtRateLimitMinutes(retryAfterSeconds) : null;
