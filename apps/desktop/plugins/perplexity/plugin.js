(function () {
  ...
  function readMoneyLike(value) {
    ...
    const cents = readNumberField(value, [
      "cents",
      "amount_cents",
      "amountCents",
      "value_cents",
  ...
  function extractAuthToken(requestHex) {
    ...
    for (let i = start; i + 1 < upper.length; i += 2) {
      ...
      if (byte === 0x5f) {
        ...
        if (next === 0x10 || next === 0x11 || next === 0x12 || next === 0x13 || next === 0x14)
          break;
      }
  ...
  function fetchJsonOptional(ctx, url, authToken, extraHeaders) {
    ...
    if (ctx.util.isAuthStatus(resp.status)) {
      if (
        resp.status === 403 &&
        typeof resp.bodyText === "string" &&
        resp.bodyText.indexOf("Just a moment") !== -1
      ) {
  ...
  function readBalanceUsd(group) {
    ...
    const tryNode = (node) => {
      ...
      for (const k in node)
        if (/(balance|credit|wallet|prepaid|available)/i.test(k)) {
  ...
  globalThis.__pacebar_plugin = { id: "perplexity", probe: probe };
})();
