---
title: "Windsurf"
description: "Windsurf — prompt credits, flex credits."
---

> Reverse-engineered from app bundle, extension.js, and language server binary. May change without notice.

Windsurf and [Antigravity](/docs/providers/antigravity) share the same Codeium language server binary and Connect-RPC protocol. The discovery, port probing, and RPC endpoints are virtually identical — the key differences are authentication (Windsurf requires an API key) and the usage model (credits vs fractions).
...
Same process as [Antigravity](/docs/providers/antigravity) — same binary, same flags. The distinguishing marker is the `--ide_name` flag value in the process args.
...
Returns plan info with credit-based usage for the current billing cycle. Same RPC as [Antigravity](/docs/providers/antigravity), but requires `metadata.apiKey`.
...
Unlike [Antigravity](/docs/providers/antigravity), Windsurf **requires** `metadata.apiKey`. The LS uses it to authenticate with the cloud backend internally. Antigravity authenticates via the Google OAuth session instead.
