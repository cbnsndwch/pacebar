...
function extractProviderUsage(snapshot) {
  ...
  for (const line of snapshot.lines) {
    ...
    if (line.type === "text" && line.label === "Today") {
      ...
      if (parsed) { tokens += parsed.tokens; dollars += parsed.dollars }
    }
  }
...
globalThis.__pacebar_plugin = {
  ...
  probe(ctx) {
    ...
    try {
      const hnResp = apiFetch(ctx, prefs.workerUrl, prefs.token, "GET", "/api/v1/hacknight/current", null)
    ...
    try {
      ...
      const lbResp = apiFetch(
        ctx, prefs.workerUrl, prefs.token,
        "GET", `/api/v1/leaderboard?window=${window}&metric=tokens`, null
      )
