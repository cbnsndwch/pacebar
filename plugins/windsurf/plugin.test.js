...
describe("windsurf plugin", () => {
  ...
  it("does not probe the local language server or localhost", async () => {
    ...
    expect(urls.every((url) => url.startsWith("https://server.self-serve.windsurf.com/"))).toBe(
      true,
    );
  ...
  it("treats SQLite read errors as missing API key and logs a warning", async () => {
    ...
    expect(ctx.host.log.warn).toHaveBeenCalledWith(
      expect.stringContaining("failed to read API key"),
    );
    expect(ctx.host.http.request).not.toHaveBeenCalled();
  });
  ...
  it("throws quota unavailable when the cloud returns only transient failures", async () => {
    ...
    expect(ctx.host.log.warn).toHaveBeenCalledWith(
      expect.stringContaining("cloud request returned status 503"),
    );
    expect(ctx.host.log.warn).toHaveBeenCalledWith(
      expect.stringContaining("cloud request returned status 502"),
    );
  });
  ...
  it("throws the login hint when every cloud response is an auth failure", async () => {
    ...
    expect(() => plugin.probe(ctx)).toThrow("Start Windsurf or sign in and try again.");
    expect(ctx.host.log.warn).toHaveBeenCalledWith(
      expect.stringContaining("cloud request returned status 401"),
    );
    expect(ctx.host.log.warn).toHaveBeenCalledWith(
      expect.stringContaining("cloud request returned status 403"),
    );
  });
  ...
  it("falls through to the next variant when a cloud request throws", async () => {
    ...
    expect(result.plan).toBe("Pro");
    expect(ctx.host.log.warn).toHaveBeenCalledWith(
      expect.stringContaining("cloud request failed for windsurf"),
    );
  });
