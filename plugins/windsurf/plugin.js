(function () {
  ...
  function callCloud(ctx, apiKey, variant) {
    try {
      ...
      if (resp.status < 200 || resp.status >= 300) {
        ctx.host.log.warn(
          "cloud request returned status " + resp.status + " for " + variant.marker,
        );
        if (
          ctx.util &&
          typeof ctx.util.isAuthStatus === "function" &&
          ctx.util.isAuthStatus(resp.status)
        ) {
          return { __pacebarAuthError: true };
        }
        return null;
      }
  ...
  function probe(ctx) {
    ...
    for (var i = 0; i < VARIANTS.length; i++) {
      ...
      if (data && data.__pacebarAuthError) {
        sawAuthFailure = true;
        continue;
      }
  ...
  globalThis.__pacebar_plugin = { id: "windsurf", probe: probe };
})();
