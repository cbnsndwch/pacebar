(function () {
  const KEYCHAIN_SERVICE = "PaceBar-copilot";
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && parsed.token) {
          ctx.host.log.info("token loaded from PaceBar keychain");
          return { token: parsed.token, source: "keychain" };
        }
      }
    } catch (e) {
      ctx.host.log.info("PaceBar keychain read failed: " + String(e));
    }
    return null;
  }
  ...
  globalThis.__pacebar_plugin = { id: "copilot", probe };
})();
