...
function makeUserStatusResponse(overrides) {
  ...
  if (overrides) {
    if (overrides.planName !== undefined)
      base.userStatus.planStatus.planInfo.planName = overrides.planName;
    if (overrides.configs !== undefined)
      base.userStatus.cascadeModelConfigData.clientModelConfigs = overrides.configs;
...
describe("antigravity plugin", () => {
  ...
  it("skips configs with missing or empty labels", async () => {
    ...
    const response = makeUserStatusResponse({
      configs: [
        ...
        {
          label: "",
          modelOrAlias: { model: "M99" },
          quotaInfo: { remainingFraction: 0.8, resetTime: "2026-02-08T09:10:56Z" },
        },
        {
          modelOrAlias: { model: "M100" },
  ...
  it("clamps remainingFraction outside 0-1 range", async () => {
    ...
    const response = makeUserStatusResponse({
      configs: [
        {
          label: "Gemini Pro (Over)",
          modelOrAlias: { model: "M1" },
          quotaInfo: { remainingFraction: 1.5, resetTime: "2026-02-08T09:10:56Z" },
        },
        {
          label: "Gemini Flash (Neg)",
          modelOrAlias: { model: "M2" },
          quotaInfo: { remainingFraction: -0.3, resetTime: "2026-02-08T09:10:56Z" },
        },
      ],
    });
    setupLsMock(ctx, discovery, response);
  ...
  it("decodes protobuf tokens from SQLite", async () => {
    const ctx = makeCtx();
    const futureExpiry = Math.floor(Date.now() / 1000) + 3600;
    const protoB64 = makeOAuthSentinelB64(ctx, {
  ...
  it("tries both tokens before refreshing", async () => {
    ...
    expect(
      capturedTokens.filter((t) => t === "Bearer ya29.cached-also-bad").length,
    ).toBeGreaterThan(0);
  ...
  it("skips expired DB access token and falls through to refresh", async () => {
    ...
    setupSqliteMock(
      ctx,
      makeOAuthSentinelB64(ctx, {
        accessToken: "ya29.expired-proto-token",
        refreshToken: "1//refresh",
  ...
  it("refresh-token-only state (no access token, no cache) refreshes and succeeds", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url.includes("oauth2.googleapis.com")) {
        ...
        return {
          status: 200,
          bodyText: JSON.stringify({ access_token: "ya29.from-refresh", expires_in: 3600 }),
        };
      }
      if (url.includes("fetchAvailableModels")) {
        capturedAuths.push(opts.headers.Authorization);
  ...
  it("expired access token + valid refresh token: refresh is called exactly once with the refreshed token", async () => {
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      ...
      if (url.includes("oauth2.googleapis.com")) {
        ...
        return {
          status: 200,
          bodyText: JSON.stringify({ access_token: "ya29.new", expires_in: 3600 }),
        };
      }
      if (url.includes("fetchAvailableModels")) {
