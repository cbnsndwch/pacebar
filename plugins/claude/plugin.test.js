...
const expectedHash = (path) => crypto.createHash("sha256").update(path).digest("hex").slice(0, 8);
...
describe("claude plugin", () => {
  ...
  it("hashes CLAUDE_CONFIG_DIR verbatim (no tilde expansion, NFC-normalized)", async () => {
    ...
    const result = plugin.probe(ctx);
    expect(result.lines.find((line) => line.label === "Session")).toBeTruthy();
    expect(ctx.host.keychain.readGenericPasswordForCurrentUser).toHaveBeenCalledWith(hashedService);
  });
  ...
  it("uses env-injected OAuth tokens without hitting /api/oauth/usage", async () => {
    ...
    expect(result.lines.find((line) => line.label === "Last 30 Days")?.value).toContain(
      "150 tokens",
    );
  });
  ...
  describe("rate limiting (429)", () => {
    ...
    it("resumes API calls after rate-limit window expires", async () => {
      ...
      try {
        ...
        expect(
          result2.lines.find((l) => l.label === "Status" && l.color === "#f59e0b"),
        ).toBeUndefined();
      } finally {
        vi.useRealTimers();
      }
