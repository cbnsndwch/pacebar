...
describe("App", () => {
  ...
  const triggerPluginContextAction = async (
    ...
    const contextAction = menuState.iconMenuItemConfigs.find(
      (item) => item.id === `ctx-${action}-${pluginId}`,
    )?.action;
  ...
  it("uses selected provider on detail view and keeps it on home/settings", async () => {
    ...
    state.probeHandlers?.onResult({
      ...
      iconUrl: "icon-b",
      lines: [
        { type: "progress", label: "Session", used: 30, limit: 100, format: { kind: "percent" } },
      ],
    });
  ...
  it("logs error when saving menubar icon style fails", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    state.saveMenubarIconStyleMock.mockRejectedValueOnce(
      new Error("save menubar icon style failed"),
    );
    ...
    await waitFor(() =>
      expect(errorSpy).toHaveBeenCalledWith(
        "Failed to save menubar icon style:",
        expect.any(Error),
      ),
    );
    errorSpy.mockRestore();
  ...
  it("respects manual refresh cooldown for sidebar context menu reload", async () => {
    ...
    state.startBatchMock.mockClear();
    const reloadAction = await triggerPluginContextAction("Beta", "b", "reload");
    const firstReloadConfig = menuState.iconMenuItemConfigs.find(
      (item) => item.id === "ctx-reload-b",
    );
    ...
    const cooldownReloadConfig = menuState.iconMenuItemConfigs.find(
      (item) => item.id === "ctx-reload-b",
    );
        ...
        return [
          {
            ...
            iconUrl: "icon-a",
            primaryProgressLabel: null,
            lines: [{ type: "text", label: "Alpha line", scope: "overview" }],
          },
          {
            id: "b",
            name: "Beta",
            iconUrl: "icon-b",
            primaryProgressLabel: null,
            lines: [{ type: "text", label: "Beta line", scope: "overview" }],
          },
        ];
      }
      return null;
  ...
  it("queues a follow-up tray update when render is in-flight", async () => {
    ...
    try {
      ...
      state.loadPluginSettingsMock.mockResolvedValueOnce({ order: ["a"], disabled: [] });
      state.renderTrayBarsIconMock.mockReturnValueOnce(firstRender).mockResolvedValueOnce({});
