...
describe("pace-status", () => {
  it("returns null for non-finite inputs", () => {
    ...
    expect(
      calculatePaceStatus(10, Number.POSITIVE_INFINITY, resetsAtMs, ONE_DAY_MS, nowMs),
    ).toBeNull();
  ...
  it("returns null when less than 5% of the period has elapsed", () => {
    ...
    expect(
      calculatePaceStatus(10, 100, resetsAtMs, periodDurationMs, beforeThresholdNowMs),
    ).toBeNull();
  });
...
describe("calculateDeficit", () => {
  it("returns null for non-finite inputs", () => {
    ...
    expect(
      calculateDeficit(10, Number.POSITIVE_INFINITY, resetsAtMs, ONE_DAY_MS, nowMs),
    ).toBeNull();
