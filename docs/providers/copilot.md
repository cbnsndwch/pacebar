...
1. **PaceBar Keychain** (`PaceBar-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the PaceBar keychain for faster access on subsequent probes.
