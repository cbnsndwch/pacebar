...
PaceBar can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.pacebar/config.json`
- Default: off
- UI: none

## Config File

Create `~/.pacebar/config.json`:
...
- Restart PaceBar after changing the file.
...
This applies to provider and plugin HTTP requests that go through PaceBar's built-in HTTP client.
